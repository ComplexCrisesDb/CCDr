#function to plot the idf for each category and group by typology and nature
idf_barplot=function(idf,vars_type=c("economic_shock","non_economic_shock","debt_outcomes","debt_structure","characteristics_program","adustment_program"),
  ...
  }else{idf=idf %>% filter(type %in% vars_type)}
  
  res[["fig"]]=ggplot(idf)+
    geom_bar(stat="identity",aes(x=reorder(variable,idf),y=idf,fill=type))+
    scale_fill_manual(values=c("darkred","blue","darkgreen","lightgrey","orange","brown"))+
    theme_bw()+
    lims(y=c(0,1))+
    labs(title="Probability of the events",x=NULL,y=NULL)+
