...
<ol>
  ...
  In the scraping process, we might end up with some URLs of problematic documents i.e. documents on which we do not want to conduct the tf extraction. In our example, we downloaded many working paperThe logical step to take would be the manually erase these documents: this, in turn, would be tedious and take a long time. <br> <br>
...
tf_path = "~/Desktop/Packages/TextMiningCrisis/data/tf_crisis_words_example.RData"
corpus_path = "~/Desktop/Packages/TextMiningCrisis/data/corpus_example.RData"

TextMiningCrisis::run_tf_update(path_tf_to_update = tf_path, corpus_path = corpus_path, keyword_list = c("Natural_disaster"))
```
